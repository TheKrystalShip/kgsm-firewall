...
/// added to the firewall without a descriptor entry fails the build here, and a descriptor entry
/// naming a variable the firewall does not read fails here too.
///
...
{
    private const string EnvPrefix = "KGSM_FIREWALL_";

    /// <summary>
    /// Variables the firewall genuinely reads that do NOT appear as literals in its source: the
    ...
    /// <summary>Every KGSM_FIREWALL_* variable named anywhere in the daemon's own source.</summary>
    private static HashSet<string> EnvKeysInSource()
    {
        string src = Path.Combine(RepoRoot(), "src", "Firewall");
        ...
        foreach (string file in Directory.EnumerateFiles(src, "*.cs", SearchOption.AllDirectories))
        {
            ...
            foreach (System.Text.RegularExpressions.Match m in
                     System.Text.RegularExpressions.Regex.Matches(File.ReadAllText(file), @"KGSM_FIREWALL_[A-Z0-9_]+"))
                found.Add(m.Value);
        }
    ...
    [Fact]
    public void Every_env_var_the_firewall_reads_is_described()
    {
        ...
        Assert.True(missing.Count == 0,
            "these variables are read by the firewall but not described in deploy/kgsm-firewall.leaf.json, so " +
    ...
    public void Every_described_env_var_is_real()
    {
        ...
        Assert.True(fabricated.Count == 0,
            "these descriptor fields name variables the firewall does not read — an override written for one " +

...
/// <summary>
/// The leaf config descriptor (<c>deploy/kgsm-firewall.leaf.json</c>) is what the Control Panel
/// renders this daemon's configuration page from. These tests are the anti-drift guard, pinning a
/// chain of three: a property on <see cref="FirewallSettings"/>, a key in
/// <c>kgsm-firewall.settings.json</c>, and a field in the descriptor. Every link is checked in both
/// directions, so a knob cannot be added to one and forgotten in the others.
///
/// The settings file is the pivot rather than a table of constants, because it is what the binary
...
public class LeafDescriptorTests
{
    ...
    /// firewall properties, so the property-level checks skip them. Named explicitly rather than
    ...
    private static string RepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "kgsm-firewall.slnx")))
            dir = dir.Parent;

        Assert.True(dir is not null, "could not locate the repo root (no kgsm-firewall.slnx above the test binary)");
        return dir!.FullName;
    }

    private static JsonElement Descriptor()
    {
        string path = Path.Combine(RepoRoot(), "deploy", "kgsm-firewall.leaf.json");
    ...
    {
        string path = Path.Combine(RepoRoot(), "src", "Firewall", "kgsm-firewall.settings.json");
    ...
    /// <summary>The env-var spelling of every property the binary binds.</summary>
    private static HashSet<string> SettingsPropertyKeys() =>
        typeof(FirewallSettings)
            .GetProperties()
            .Select(p => $"{FirewallSettings.Section}__{p.Name}")
    ...
    {
        string path = Path.Combine(RepoRoot(), "deploy", "kgsm-firewall.env.example");
        ...
        Assert.True(unknown.Count == 0,
            "deploy/kgsm-firewall.env.example sets these, and kgsm-firewall.settings.json declares no such key, so " +
    ...
    public void Every_configurable_key_is_described()
    {
        ...
        Assert.True(missing.Count == 0,
            "these keys are settable but not described in deploy/kgsm-firewall.leaf.json, so the Control Panel " +
    ...
    public void Every_settings_key_binds_to_a_property()
    {
        ...
        Assert.True(unbound.Count == 0,
            "these keys are declared in kgsm-firewall.settings.json but have no matching property on " +
            "FirewallSettings, so binding silently drops them:\n  " + string.Join("\n  ", unbound));
    }
    ...
    public void Every_settings_property_is_declared_in_the_file()
    {
        ...
        Assert.True(undeclared.Count == 0,
            "these FirewallSettings properties are missing from kgsm-firewall.settings.json, which is supposed to " +
    ...
    public void Descriptor_identity_matches_this_project()
    {
        ...
        Assert.Equal("firewall", d.GetProperty("id").GetString());
        Assert.Equal("kgsm-firewall.service", d.GetProperty("unit").GetString());
        Assert.Equal("restart", d.GetProperty("applyMode").GetString());
        // Socket-activated and idle-exiting: inactive is its resting state, and the post-restart
        // canary has to know that or an ordinary apply reads as a failed leaf.
        Assert.True(d.GetProperty("onDemand").GetBoolean());
    ...
    public void Floor_sources_are_declared_and_typed()
    {
        ...
        Assert.NotEmpty(sources);   // the firewall's floor is its unit's Environment= lines + the optional env file
    ...
    public void Wire_keys_already_in_use_by_the_api_are_preserved()
    {
        ...
        // and silently reverts the firewall to its floor, so they are pinned here deliberately.
        var keys = Fields().Select(f => Str(f, "key")).ToHashSet(StringComparer.Ordinal);
